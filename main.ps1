...
if($connectedDevices.Count -eq 1){
    ...
    if($javaPath){
        Write-Host "[SUCCESS] Found Java"
        $apkPath = Read-Host -Prompt "Drag and drop the APK you want to use onto this terminal"
        ...
        if($apkPath.Contains(".apk") -And $apkExists){
            ...
            $obbPath = Read-Host -Prompt "Drag and drop the OBB you want to use onto this terminal, or press enter to skip"
